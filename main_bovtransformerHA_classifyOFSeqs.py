...
log_path = "logs/bovtrans/HA_of20_Hidden200_C1000" #_C1000_pretrained"
...
#                targets = targets.t().contiguous().view(-1)
                ...
                output = F.softmax(output.view(-1, output.shape[-1]), dim=1) # To (BATCH*SEQ_SIZE, NCLUSTERS)
...
#        targets = targets.t().contiguous().view(-1)
        ...
        with torch.set_grad_enabled(phase == 'train'):
            ...
            for i, vid in enumerate(vid_path):
                stroke_ids.extend([vid+"_"+str(stroke[0][i].item())+"_"+str(stroke[1][i].item())] * inputs.size(0))
...
#    # Uncomment only while training.
    ...
    train_dataset = StrokeFeatureSequenceDataset(ft_path, train_lst, DATASET, LABELS, CLASS_IDS, 
                                         frames_per_clip=SEQ_SIZE, extracted_frames_per_clip=2,
                                         step_between_clips=STEP, train=True)
    val_dataset = StrokeFeatureSequenceDataset(ft_path_test, test_lst, DATASET, LABELS, CLASS_IDS, 
                                         frames_per_clip=SEQ_SIZE, extracted_frames_per_clip=2,
    ...
    model = tt.TransformerModel(ntokens, emsize, nhead, nhid, nlayers, dropout)
...
if __name__ == '__main__':
    ...
    print("OF20 BOV Transformer HA with Embedding (Classify Sequences) ...")
