...
@Description: Finetuning Transformer model for downstream task of SA sequence classification. 
...
log_path = "logs/bovtrans_selfsup/Down_SA_of20_C300_PreC"+str(cluster_size)+"_F"+str(fstep)
model_path = "logs/bovtrans_selfsup/SA_of20_Hidden200_C300_HardF"+str(fstep)  #siamtrans_ep60_S30C200_SGD.pt
...
#    boundaries = read_boundaries(vkeys, HIST_DIFFS, SBD_MODEL)
    ...
    for i, pr in enumerate(pred_list):
        if prev_gt != stroke_ids[i]:
            ...
            print("Preds {} : {} :: {}".format(tm+1, Counter(vid_preds), pred_labels[-1]))
...
def save_model_checkpoint(base_name, model, ep, opt):
    ...
    model_name = os.path.join(base_name, "siamtrans_ep"+str(ep)+"_"+opt+".pt")
...
#    ft_path_val = os.path.join(base_name, feat_path, feat_val)
    ...
    model = siamese_net.SiameseTransformerSANet(ntokens, emsize, nhead, nhid, nlayers, dropout)
...
if __name__ == '__main__':
    ...
    print("Siamese Transformer SA model finetuning on OF20 Ang sequences...")
