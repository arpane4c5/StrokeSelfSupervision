...
@Description: Finetuning Transformer model for downstream task of sequence classification.
"""
...
log_path = "logs/bovtrans_seq2seq/Down_SA_of20_Hidden200_Pre_C"+str(cluster_size)
model_path = "logs/bovtrans_seq2seq/SA_of20_Hidden200_C"+str(cluster_size)+"_SSFuturePred/"#transformer_ep30_S30C300_SGD.pt"
...
#                inputs = inp_emb.to(device)                     # comment out for SA
                inputs = inputs.permute(1, 0, 2).contiguous().to(device)       # Convert to (SEQ, BATCH, DIM)
...
#    # Uncomment only while training.
    ...
    val_dataset = StrokeFeaturePairsDataset(ft_path_test, test_lst, DATASET, LABELS, CLASS_IDS, 
                                         frames_per_clip=SEQ_SIZE, extracted_frames_per_clip=2,
    ...
    model = tt.TransformerModelSA(ntokens, emsize, nhead, nhid, nlayers, #num_classes, 
                                dropout)
    model = load_weights(model_path, model, N_EPOCHS, 
                                    "S30"+"C"+str(cluster_size)+"_Adam")

#    model = tt.TransformerModelClassifier(ntokens, emsize, nhead, nhid, nlayers, 
#                                              num_classes, dropout)
...
#                                     "S"+str(SEQ_SIZE)+"C"+str(cluster_size)+"_Adam")
...
#    ANNOTATION_FILE = "/home/arpan/VisionWorkspace/Cricket/stroke_recognition/config/stroke_types_classes.txt"
    ...
    print("OF20 BOV Transformer SA without Embedding (Downstream Finetuning of Seq2Seq Model...")
