...
# -*- coding: utf-8 -*-
"""
Created on Fri July 23 10:39:42 2021
...
#    import cv2
    ...
    inputs1, inputs2, *_ = dataset.__getitem__(801)
...
#    f, axarr = plt.subplots(4,4)
    for eg, inputs in enumerate([inputs1, inputs2]):
    ...
    #        axarr[i][j].imshow(np.array(t, dtype=np.uint8))
            ...
            plt.savefig(os.path.join(log_path, "visual_samps{}_{}.png".format(eg, i))) #, bbox_inches='tight')
    plt.close('all')
...
def save_model_checkpoint(base_name, model, ep, opt):
    ...
    if torch.cuda.device_count() > 1:
        print("Unwrapp from DataParallel.")
...
#    best_acc = 0.0
#    vis_samples(dataloaders['train'].dataset, True)
...
#                output2 = output2.view(-1, output2.shape[-1])
                if output1.shape[-1]==1:
                    loss = criterion(torch.sigmoid(output1.squeeze(axis=2)), 
                                     torch.sigmoid(output2.squeeze(axis=2)), 
                                     labels)
                else:
                    loss = criterion(torch.sigmoid(output1), 
...
#                    break
            ...
            epoch_loss = running_loss / (x1.shape[1] * (bno+1)) #len(dataloaders[phase].dataset)
...
if __name__ == '__main__':
    ...
    for SEQ_SIZE in seq_sizes:
        ...
        acc.append(main(DATASET, LABELS, CLASS_IDS, BATCH_SIZE, SEQ_SIZE, STEP, 
                        nstrokes=-1, N_EPOCHS=N_EPOCHS, base_name=base_path))
